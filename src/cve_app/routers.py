...
from src.cve_app.schemas import CVERecord, CVERecordCreate
...
@cve_app.post("/cve/", response_model=CVERecord)
...
async def update_cve(cve_id: str, updates: CVERecordCreate, db: AsyncSession = Depends(get_db)):
    db_cve = await get_cve_record(db, cve_id)
    if db_cve is None:
        raise CVENotFoundException()
    await update_cve_record(db, cve_id, updates.dict())
